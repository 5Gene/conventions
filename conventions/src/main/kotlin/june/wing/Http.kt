...
interface Http {
    companion object {
        const val FORMAT_DATA_LINE_END = "\r\n"
    }
    ...
    interface MultipartBody : Http {
        ...
        fun writeTo(out: OutputStream) {
            val contentDisposition = "${contentDisposition()}$FORMAT_DATA_LINE_END"
            out.write(contentDisposition.toByteArray())
            print(contentDisposition)
            contentType()?.let {
                val contentType = "$it$FORMAT_DATA_LINE_END"
                out.write(contentType.toByteArray())
                print(contentType)
            }
            contentLength()?.let {
                val contentLength = "${it}$FORMAT_DATA_LINE_END"
                out.write(contentLength.toByteArray())
                print(contentLength)
            }
            out.write(FORMAT_DATA_LINE_END.toByteArray())
            println(FORMAT_DATA_LINE_END)
        }
    }

    open class MultipartStringBody(val key: String, val value: String) : MultipartBody, Http {
        ...
        override fun writeTo(out: OutputStream) {
            super.writeTo(out)
            out.write("$value$FORMAT_DATA_LINE_END".toByteArray())
    ...
    class MultipartFileBody(private val key: String, private val file: File) : MultipartBody, Http {
...
//            val boundary = "xxxxxxxxxxxxxxxxxxxxxxxxxxx"
                ...
                conn.outputStream.use {
                    //开始只有一个--也就是--boundary\r\n
                    val headBoundary = (twoHyphens + boundary + FORMAT_DATA_LINE_END).toByteArray()
                    request.multipartBodys.forEach { multipartBody ->
                        it.write(headBoundary)
                        multipartBody.writeTo(out = it)
                    }
                    //最后必须是 --boundary--\r\n 结束 注意首位的--
                    val bottomBoundary = (twoHyphens + boundary + twoHyphens + FORMAT_DATA_LINE_END).toByteArray()
