package june.knife.asm.visitors

import june.knife.asm.Action
import june.knife.asm.asmLog
import june.knife.asm.findAction
import june.knife.asm.insertDefReturn
import june.knife.asm.necessaryStackAndLocals
import june.wing.lightRed
...
internal class RemoveInvokeMethodVisitor(
    ...
    override fun visitMethodInsn(
        ...
        val methodAction = methodActions.findAction(owner, name, descriptor)
...
internal class ChangeInvokeOwnerMethodVisitor(
    ...
    override fun visitMethodInsn(
        ...
        val methodAction: Action.ChangeInvoke? = methodActions.findAction(owner, name, descriptor)
        if (methodAction == null) {
            //没匹配到就不需要处理
            super.visitMethodInsn(opcode, owner, name, descriptor, isInterface)
        } else {
